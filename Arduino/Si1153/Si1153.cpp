 ...
 */
void Si1153::write_byte(uint8_t addr, uint8_t reg, uint8_t data){
 ...
 */
int Si1153::read_byte(uint8_t addr, uint8_t reg){
...
void Si1153::param_set(uint8_t loc, uint8_t val){
	...
	while(1){
		...
		int CMMND_CTR = Si1153::read_byte(Si1153::DEVICE_ADDRESS,Si1153::RESPONSE0); 

        //Write data to INPUT0 register
		Si1153::write_byte(Si1153::DEVICE_ADDRESS,Si1153::HOSTIN0,val);

        //Write location (location of param to set) to COMMAND Register
        uint8_t location = loc | (0B10<<6);
		Si1153::write_byte(Si1153::DEVICE_ADDRESS,Si1153::COMMAND,location);

        //If response has incremented then Write was Successful
		int response = Si1153::read_byte(Si1153::DEVICE_ADDRESS,Si1153::RESPONSE0);
...
int Si1153::param_query(uint8_t loc){
	...
	while(1){
		...
		if(response>CMMND_CTR){
			...
			result = Si1153::read_byte(Si1153::DEVICE_ADDRESS,Si1153::RESPONSE1);
			break;
		}
	}	
	return result;
...
void Si1153::send_command(uint8_t code){
    ...
    while(1){
        ...
        //Check the current state of Command_CTR register. This will be used to see if write was successful.
        int CMMD_CTR = Si1153::read_byte(Si1153::DEVICE_ADDRESS, Si1153::RESPONSE0);
      
        //Write the Command to the Command Register
        Si1153::write_byte(Si1153::DEVICE_ADDRESS, Si1153::COMMAND,code);

 ...
 */
void VL53L0::write_byte(uint8_t addr, uint8_t subAddress, uint8_t data){
 ...
 */
int VL53L0::read_byte(uint8_t addr, uint8_t subAddress){
 ...
 */
 ...
 int VL53L0::read_2bytes(uint8_t addr, uint8_t subAddress){
    int val = -1;
    Wire.beginTransmission(addr);        //Send Start Signal and Address over i2c
    Wire.write(subAddress);              //SubAddress is the register to read from
    Wire.endTransmission();

...
#define DEVICE_NAME                     "PureEngineering-DW1000"                               /**< Name of device. Will be included in the advertising data. */
...
#define TWI_SCL_M                28   //!< Master SCL pin
#define TWI_SDA_M                29   //!< Master SDA pin                                       /**< UART RX buffer size. */
...
#define INTERFACE_NUMBER 0 //0 for dwm board, 1 for dw1000 puremodule, 2 for esp32
...
int SPI_waitforRX(uint8_t* data, uint16_t* length, uint16_t exp_length){
...
void print_to_ble(){
    if(lis2de_on){
        ...
        //dwm_gpio_cfg_output(m_nus,DWM_GPIO_IDX_13,true);
    }
...
int main(void)
{
    ...
    //TODO:check this is initializing correctly
    dwm_init();
    ...
    if(anchor){
        ...
        dwm_cfg_anchor_set(m_nus,&cfg_anchor);
        nrf_delay_ms(2000);
        dwm_cfg_get(m_nus,&cfg_node);   
    }
    else{
    ...
    for (;;)
    {
       ...
       nrf_delay_ms(1000);
        ...
        if(dwm_loc_get(m_nus,&loc) == RV_OK)
          {
            //HAL_Print("\t[%d,%d,%d,%u]\n", loc.p_pos->x, loc.p_pos->y, loc.p_pos->z,loc.p_pos->qf);

            sprintf((char *)ble_string, "\t[%lu,%lu,%lu,%u]\n",loc.p_pos->x, loc.p_pos->y, loc.p_pos->z,loc.p_pos->qf);
            send_ble_data(m_nus,(uint8_t *)ble_string,lengthble);
            ...
            {
                //HAL_Print("\t%u)", i);
                sprintf((char *)ble_string, "\t%u)",i);
                send_ble_data(m_nus,(uint8_t *)ble_string,lengthble);

                //HAL_Print("0x%llx", loc.anchors.dist.addr[i]);
                sprintf((char *)ble_string, "0x%llx", loc.anchors.dist.addr[i]);
                send_ble_data(m_nus,(uint8_t *)ble_string,lengthble);
                ...
                if (i < loc.anchors.an_pos.cnt) 
                {
                   //HAL_Print("[%d,%d,%d,%u]", loc.anchors.an_pos.pos[i].x,
                    ...
                    sprintf((char *)ble_string, "[%lu,%lu,%lu,%u]", loc.anchors.an_pos.pos[i].x,
                         loc.anchors.an_pos.pos[i].y,
                         loc.anchors.an_pos.pos[i].z,
                         loc.anchors.an_pos.pos[i].qf);
                    send_ble_data(m_nus,(uint8_t *)ble_string,lengthble);

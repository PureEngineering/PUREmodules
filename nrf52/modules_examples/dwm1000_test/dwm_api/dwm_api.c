 ...
 * @return Error code
 */
int DWM_CheckGPIOIdx(dwm_gpio_idx_t idx)
{
...
int dwm_pos_set(ble_nus_t m_nus,dwm_pos_t* pos)
{
   ...
   *(uint32_t*)(tx_data+tx_len) = pos->z;
   ...
   IF_Tx(m_nus,tx_data, &tx_len);      
   return SPI_waitforRX(m_nus,rx_data, &rx_len, 3);
}

int dwm_pos_get(ble_nus_t m_nus,dwm_pos_t* p_pos)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   IF_Tx(m_nus,tx_data, &tx_len);   
   if(SPI_waitforRX(m_nus,rx_data, &rx_len, 18) == RV_OK)
   {
      data_cnt = 5;
...
int dwm_upd_rate_set(ble_nus_t m_nus,uint16_t ur, uint16_t ur_static)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_upd_rate_get(ble_nus_t m_nus,uint16_t *ur, uint16_t *ur_static)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   IF_Tx(m_nus,tx_data, &tx_len);   
   if(SPI_waitforRX(m_nus,rx_data, &rx_len, 9) == RV_OK)
   {
      data_cnt = 5;
...
int dwm_cfg_tag_set(ble_nus_t m_nus,dwm_cfg_tag_t* cfg)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   IF_Tx(m_nus,tx_data, &tx_len); 
   //dw1000_SPI_TX(tx_data,&tx_len);
   return SPI_waitforRX(m_nus,rx_data, &rx_len, 3);
}

int dwm_cfg_anchor_set(ble_nus_t m_nus,dwm_cfg_anchor_t* cfg)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_cfg_get(ble_nus_t m_nus,dwm_cfg_t* cfg)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   IF_Tx(m_nus,tx_data, &tx_len);   
   if(SPI_waitforRX(m_nus,rx_data, &rx_len, 7) == RV_OK)
   {
...
int dwm_sleep(ble_nus_t m_nus) //void
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_loc_get(ble_nus_t m_nus,dwm_loc_data_t* loc)
{ 
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   IF_Tx(m_nus,tx_data, &tx_len);   
   if(SPI_waitforRX(m_nus,rx_data, &rx_len, DWM1001_TLV_MAX_SIZE) == RV_OK)
   {
...
int dwm_baddr_set(ble_nus_t m_nus,dwm_baddr_t* p_baddr)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_baddr_get(ble_nus_t m_nus,dwm_baddr_t* p_baddr)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   IF_Tx(m_nus,tx_data, &tx_len);   
   if(SPI_waitforRX(m_nus,rx_data, &rx_len, 11) == RV_OK)
   {
      data_cnt = 5;
...
int dwm_reset(ble_nus_t m_nus) //void
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_ver_get(ble_nus_t m_nus,dwm_ver_t* ver)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   IF_Tx(m_nus,tx_data, &tx_len);   
   if(SPI_waitforRX(m_nus,rx_data, &rx_len, 21) == RV_OK)
   {
      data_cnt = 3;      
      // read fw_version
...
int dwm_gpio_cfg_output(ble_nus_t m_nus,dwm_gpio_idx_t idx, bool value)
{      
   //if(SPI_CheckGPIOIdx(idx)!= RV_OK)
   //{
   //   return RV_ERR;
   //}
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   ...
   IF_Tx(m_nus,tx_data, &tx_len);      
   return IF_WaitRX(m_nus,rx_data, &rx_len, 3);
}
...
{      
    ...
    sprintf((char *)ble_string, "inside output \r\n");
   ...
   //}
    ...
    sprintf((char *)ble_string, "before send \r\n");
    send_ble_data(m_nus,(uint8_t *)ble_string,lengthble); 

    spiWrite(m_nus,tx_data, &tx_len);   

    sprintf((char *)ble_string, "after TX \r\n");
    send_ble_data2(m_nus,(uint8_t *)ble_string,lengthble);

    int resp = SPI_waitforRX(rx_data,&rx_len,3);
    
    sprintf((char *)ble_string, "after WAIT \r\n");
    send_ble_data2(m_nus,(uint8_t *)ble_string,lengthble);
...
int dwm_gpio_cfg_input(ble_nus_t m_nus,dwm_gpio_idx_t idx, dwm_gpio_pin_pull_t pull_mode)
{      
   //if(SPI_CheckGPIOIdx(idx)!= RV_OK)
   //{
   //   return RV_ERR;
...
int dwm_gpio_value_set(ble_nus_t m_nus,dwm_gpio_idx_t idx, bool value)
{
   //if(SPI_CheckGPIOIdx(idx)!= RV_OK)
   //{
   //  return RV_ERR;
...
int dwm_gpio_value_get(ble_nus_t m_nus,dwm_gpio_idx_t idx, bool* p_value)
{
   //if(SPI_CheckGPIOIdx(idx)!= RV_OK)
   //{
   //   return RV_ERR;
   ...
   tx_data[tx_len++] = idx & 0xff;  
   IF_Tx(m_nus,tx_data, &tx_len);   
   if(SPI_waitforRX(m_nus,rx_data, &rx_len, 6) == RV_OK)
   {
...
int dwm_gpio_value_toggle(ble_nus_t m_nus,dwm_gpio_idx_t idx)
{
   //if(SPI_CheckGPIOIdx(idx)!= RV_OK)
   //{
   //   return RV_ERR;
...
int dwm_status_get(ble_nus_t m_nus,dwm_status_t* p_status)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_int_cfg(ble_nus_t m_nus,uint8_t value)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;

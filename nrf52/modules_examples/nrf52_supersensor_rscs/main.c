...
#define SPEED_AND_CADENCE_MEAS_INTERVAL 100                                        /**< Speed and cadence measurement interval (milliseconds). */
...
int8_t new_X = 0;
int8_t net_X = 0;
uint32_t avg_stride_length = 125;
...
static void rsc_sim_measurement(ble_rscs_meas_t * p_measurement)
{
    ...
    old_X = new_X;
    new_X = lis2de_readOUT_X(m_twi_master);
    ...
    p_measurement->inst_speed = (uint32_t)(new_X>>3);
    p_measurement->inst_stride_length = avg_stride_length;
...
static void rsc_meas_timeout_handler(void * p_context)
{
    ...
    //This line sends the data. rscs_measurement holds the data. 

...
#define DEVICE_NAME                     "DW1000"                               /**< Name of device. Will be included in the advertising data. */
...
char dw1000_rx_buffer[MAX_QUEUE_SIZE];
...
static void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
    ...
    //Code to put ble received messages into ble rx queue
...
static void uart_init(void)
{
    ...
    {
        RX_PIN_NUMBER, //TODO: check this pin
        TX_PIN_NUMBER,
        RTS_PIN_NUMBER,
        CTS_PIN_NUMBER,
...
void bleTxProcessQueue(void)
{
	...
	if(numToSend)
	{
		ble_nus_string_send(&m_nus, (uint8_t *)send_buffer, numToSend);
	}
}
...
void app_loop(void)
{
	...
	//loraTx( testdata, 5); //just for testing.

	//loraRx(); //check for LoRa RX. 

	while(get_queue(&uart_rx_queue,&rxChar) )
	{
		//put_queue(&lora_tx_queue,rxChar);
	...
	while(get_queue(&dw1000_rx_queue,&rxChar) )
	{
	//	app_uart_put(rxChar);
		put_queue(&ble_tx_queue,rxChar);
	}
...
void print_to_ble(){
    if(ble_on){
    	//sprintf((char *)ble_string, "dw1000 inside print: %x \r\n",0x54);
...
{   
    ...
    APP_ERROR_CHECK(err_code);
     ...
     err_code = app_timer_start(sensor_loop_timer_id, APP_TIMER_TICKS(1200, APP_TIMER_PRESCALER), NULL);
     APP_ERROR_CHECK(err_code);
}



uint8_t dw1000_whoami(){
	uint8_t id = spiRead(0x00);
	return id;
}
...
int main(void)
{
    ...
    char ble_string[length];

    /* Initializing SPI interface for DW1000 */
    // Initialize.
    ...
    {
        sprintf(ble_string, "dw1000 Failed Config: %x \r\n",0x00);
        ble_nus_string_send(&m_nus, (uint8_t *)ble_string, length);


    }else
    {
        sprintf(ble_string, "dw1000 Config Success: %x \r\n",0x01);
        ble_nus_string_send(&m_nus, (uint8_t *)ble_string, length);
    }
	...
	//TODO: check pin8 functionality
	//nrf_gpio_cfg_output(8);
	//nrf_gpio_pin_clear(8);  //turn off GPS
	...
	//nrf_gpio_pin_set(14);  //reset Lora
	//DEBUG_PRINTF(" Reset\n\r");
    ...
    for (;;)
    {
    	...
    	//for (uint32_t i = 0; i < length; i++)
		//{
			//put_queue(&dw1000_rx_queue,ble_string[i]);
		//}
	//err_code = sd_app_evt_wait();
	//APP_ERROR_CHECK(err_code);
    //send_ble_data(m_nus,(uint8_t *)ble_string,length);
	//NRF_LOG_RAW_INFO("DW1000 talking \r\n");
    	uint8_t id = dw1000_whoami();
    	sprintf(ble_string, "dw1000 id: %x \r\n",id);
		ble_nus_string_send(&m_nus, (uint8_t *)ble_string, length);
